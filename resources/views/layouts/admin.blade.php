...
<body class="bg-gray-50 font-sans text-gray-900 antialiased">
    <div class="flex h-screen overflow-hidden" 
        ...
        <!-- Sidebar -->
        <aside class="flex flex-col sidebar-gradient text-white shadow-2xl overflow-y-auto sidebar-scroll sidebar-transition z-30"
            ...
            <nav class="flex-grow p-4 space-y-1">
                <a href="{{ route('admin.dashboard') }}" 
                   class="nav-item flex items-center p-3 text-sm font-medium rounded-lg sidebar-transition
                    ...
                    <span class="sidebar-transition" :class="sidebarOpen ? 'opacity-100' : 'opacity-0 w-0 hidden'">Dashboard</span>
                </a>
                ...
                <a href="{{ route('admin.user') }}" 
                   class="nav-item flex items-center p-3 text-sm font-medium rounded-lg sidebar-transition
                    ...
                    <span class="sidebar-transition" :class="sidebarOpen ? 'opacity-100' : 'opacity-0 w-0 hidden'">Accounts</span>
                </a>

                <a href="{{ route('admin.report') }}" 
                   class="nav-item flex items-center p-3 text-sm font-medium rounded-lg sidebar-transition
                          @if(request()->routeIs('admin.report')) nav-item-active @endif">
                    <i data-lucide="shield-check" class="w-5 h-5 flex-shrink-0" :class="sidebarOpen ? 'mr-3' : 'mr-0'"></i>
                    <span class="sidebar-transition" :class="sidebarOpen ? 'opacity-100' : 'opacity-0 w-0 hidden'">Content Moderation</span>
                </a>
            </nav>
            ...
            <div class="p-4 border-t border-white border-opacity-20">
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    ...
                    <button type="submit" 
                        ...
                        <i data-lucide="log-out" class="w-5 h-5 flex-shrink-0 group-hover:animate-pulse" :class="sidebarOpen ? 'mr-3' : 'mr-0'"></i>
                        <span class="sidebar-transition" :class="sidebarOpen ? 'opacity-100' : 'opacity-0 w-0 hidden'">Logout</span>
                    </button>
                </form>
            </div>
        </aside>
